...
SVGElement.prototype = {
	...
	attr: function (hash, val) {
		var wrapper = this,
			...
			nodeName = element.nodeName.toLowerCase(), // Android2 requires lower for "name"
		...
		// setter
			...
			for (key in hash) {
				...
				if (result !== false) {
					...
					if (key === 'text') {
						// Delete bBox memo when the name changes
	...
	*/
	...
	/**
	 * Apply CSS to HTML elements. This is used in name within SVG rendering and
	 * by the VML renderer
	 */
	...
	htmlUpdateTransform: function () {
		...
		if (elem.tagName === 'SPAN') {
			...
			if (currentTextTransform !== wrapper.cTT) { // do the calculations and DOM access only if properties changed
				...
				// correct for the length/height of the name
				if (nonLeft) {
	...
	updateTransform: function () {
		...
		} else if (rotation) { // name rotation
...
SVGRenderer.prototype = {
	 ...
	 *
	 * @param {Object} textNode The parent name SVG node
	 */
				...
				if (span !== '' || spans.length === 1) {
					...
					if (span !== ' ') {
						...
						if (!spanNo && lineNo) {
							...
							// the name element or the tspan element
							attr(
								tspan,
								'dy',
	...
	/**
	 * Add name to the SVG object
	 * @param {String} str
	 * @param {Number} x Left position
	 * @param {Number} y Top position
	 * @param {Boolean} useHTML Use HTML to render the name
	 */
	...
	/**
	 * Create HTML name node. This is used by the VML renderer as well as the SVG
	...
	/**
	 * Add a label, a name item that can hold a colored or gradient background
	 ...
	 * @param {Boolean} baseline Whether to position the label relative to the name baseline,
	 *    like renderer.name, or to the upper border of the rectangle.
	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		 ...
		 * available), and after the name of the label is updated to detect the new bounding
		...
		/**
		 * This function runs after setting name or padding, but only if padding is changed
		 */
		...
		/**
		 * After the name element is added, get the desired size of the border box
		 * and add it before the name in the DOM.
		 */
		...
		attrSetters.align = function (value) {
			...
			return false; // prevent setting name-anchor on the group
		};

		// apply these to the box and the name alike
		...
		// apply these to the box but not to the name
		...
		// Redirect certain methods to either the box or the name
		var baseCss = wrapper.css;
		return extend(wrapper, {
			/**
			 * Pick up some properties and apply them to the name instead of the wrapper
			 */
			css: function (styles) {
				if (styles) {
					var textStyles = {};
...
Highcharts.VMLElement = VMLElement = {
	...
	add: function (parent) {
		...
		// align name after adding to be able to read offset
		wrapper.added = true;
		...
		// setter
			...
			for (key in hash) {
				...
				if (result !== false && value !== null) { // #620
					...
					// name for rotated and non-rotated elements
					} else if (key === 'text') {
						this.bBox = null;
						element.innerHTML = value;
						skipAttr = true;
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	/**
	 * Create rotated and aligned name
	 * @param {String} str
	 * @param {Number} x
	 * @param {Number} y
	 */
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// The align options and name align varies on whether the stack is negative and
	...
	render: function (group) {
		var options = this.options,
			...
			str = formatOption ?
				...
				options.formatter.call(this);  // format the name in the label

		// Change the name to reflect the new total and set visibility to hidden in case the serie is hidden
		if (this.label) {
		...
		// Create new label
			...
			this.label =
				this.axis.chart.renderer.text(str, 0, 0, options.useHTML)		// dummy positions, actual position updated with setOffset method in columnseries
					...
					.attr({
						align: this.textAlign,				// fix the name-anchor
...
Tooltip.prototype = {
	...
	/**
	 * Refresh the tooltip's name and position.
	 * @param {Object} point
	 */
...
Legend.prototype = {
	...
	init: function (chart, options) {
		...
		legend.initialItemY = padding - 5; // 5 is the number of pixels above the name
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			// A group to hold the symbol and name. Text is to be appended in Legend class.
			...
			// Generate the list item name and add it to the group
	...
	scroll: function (scrollBy, animation) {
		...
		if (currentPage > 0) {
			...
			this.down.attr({
					x: 18 + this.pager.getBBox().width, // adjust to name width
...
Chart.prototype = {
	...
	redraw: function (animation) {
		...
		// Adjust title layout (reflow multiline name)
		chart.layOutTitles();
	...
	/**
	 * Dim the chart and show a loading name or symbol
	 * @param {String} str An optional name to show in the loading label instead of the default one
	 */
...
Point.prototype = {
	 ...
	 *
	 * @return {String} A string to be concatenated in to the common tooltip name
	 */
...
Series.prototype = {
	...
	alignDataLabel: function (point, dataLabel, options, alignTo, isNew) {
		...
		if (visible) {
			...
			// Add the name size for alignment calculation
			extend(options, {
				width: bBox.width,
				height: bBox.height
			});
			...
			if (options.rotation) { // Fancy box alignment isn't supported for rotated name
				alignAttr = {
					align: options.align,
		...
		while (i--) {
				...
				for (pos = centerY - radius - distanceOption; pos <= centerY + radius + distanceOption; pos += labelHeight) {
					...
					if (!isNaN(slotX)) {
						chart.renderer.rect(slotX, slotY - 7, 100, labelHeight, 1)
							...
							.add();
						chart.renderer.name('Slot '+ (slots.length - 1), slotX, slotY + 4)
							.attr({
								fill: 'silver'
							}).add();
					}
			...
			for (j = 0; j < length; j++) {
				...
				dataLabel._pos = {
					...
					y: y + options.y - 10 // 10 is for the baseline (label vs name)
				};
				dataLabel.connX = x;
				dataLabel.connY = y;

...
(function(window) {
  ...
  ZeroClipboard.prototype.setText = function(text) {
    ZeroClipboard.setData("name/plain", text);
    return this;
  };
  ZeroClipboard.prototype.setHtml = function(html) {
    ZeroClipboard.setData("name/html", html);
    return this;
  };
  ...
  var _preprocessEvent = function(event) {
    ...
    switch (event.type) {
     ...
     case "copy":
      ...
      if (!(_clipData["name/html"] || _clipData["name/plain"]) && targetEl && (htmlContent = targetEl.value || targetEl.outerHTML || targetEl.innerHTML) && (textContent = targetEl.value || targetEl.textContent || targetEl.innerText)) {
        event.clipboardData.clearData();
        event.clipboardData.setData("name/plain", textContent);
        if (htmlContent !== textContent) {
          event.clipboardData.setData("name/html", htmlContent);
        }
      } else if (!_clipData["name/plain"] && event.target && (textContent = event.target.getAttribute("data-clipboard-name"))) {
        event.clipboardData.clearData();
        event.clipboardData.setData("name/plain", textContent);
      }
      break;

...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '文件已经被加入处理队列:';
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							this.queueData.errorMsg += '\n最大只允许选择 (' + errorMsg + ')个文件.';
						} else {
							this.queueData.errorMsg += '\n最大只允许 (' + settings.queueSizeLimit + ')个文件同时加入队列.';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n 文件 "' + file.name + '" 超过文件大小限制 (' + settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n 文件 "' + file.name + '" 为空.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n 文件 "' + file.name + '" 不是允许上传的文件类型 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
